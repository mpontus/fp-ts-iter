...
/**
 * `AsyncIter` represents an async generator function with no arguments or
 * `Lazy<AsyncIterable<A>>`.
 *
 * @since 0.1.0
 * @category Model
 * @example
 *   import { AsyncIter } from "fp-ts-iter/lib/AsyncIter";
 *
 *   const generate_numbers: AsyncIter<number> = async function* () {
 *     yield 1;
 *     yield 2;
 *     yield 3;
 *   };
...
async function* _concat<A>(iterables: AsyncIterable<A>[]): AsyncIterable<A> {
  const asyncIterators = Array.from(iterables, (o) =>
